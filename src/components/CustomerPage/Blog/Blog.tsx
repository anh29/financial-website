...
const Blog: React.FC = () => {
  ...
  const handleReadMore = (postId: number) => {
    navigate(`/customer/blog/${postId}`)
  }

  return (
    <div className={styles.blogPage}>
      <button className={styles.backButton} onClick={() => navigate('/customer')}>
        <FaArrowLeft /> Quay lại
      </button>

      <div className={styles.blogHeader}>
        <h1>Blog</h1>
        <p className={styles.subtitle}>Khám phá kiến thức tài chính và cập nhật tin tức mới nhất</p>
      </div>
        ...
        {blogPosts.map((post) => (
          <article key={post.id} className={styles.blogCard}>
            ...
            <div className={styles.blogContent}>
              ...
              <h2>{post.title}</h2>
              <p>{post.excerpt}</p>
              <button className={styles.readMoreButton} onClick={() => handleReadMore(post.id)}>
                Đọc thêm
              </button>
            </div>
          </article>
